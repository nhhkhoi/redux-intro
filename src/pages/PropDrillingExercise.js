...
const RootComponent = (props) => {
  ...
  const addProductToCart = (newProduct) => {
    const addNewProduct = cart.products.map((product) => {
    ...
    let newTotalCartPrice = (cart.totalPrice += newProduct.price);
    setCart({ products: addNewProduct, totalPrice: newTotalCartPrice });
  };
  ...
  const removeProductFromCart = (removeProduct) => {
    let newTotalCartPrice = cart.totalPrice;
    const removeNewProduct = cart.products.map((product) => {
      if (product.id === removeProduct.id && product.qty > 0) {
        product.qty -= 1;
        product.price -= removeProduct.price;
        newTotalCartPrice -= removeProduct.price;
      }
      return product;
    });
    setCart({ products: removeNewProduct, totalPrice: newTotalCartPrice });
  };
